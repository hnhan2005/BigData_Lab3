...
> **Reference**: [Main Spec File](./spec-[feature-slug]-[status].md)

## Spec Goal
[High-level summary of the spec's main objective. Focus on the "Why" and the primary value deliverable.]
*Example: Focus on optimizing system performance and distinguishing features between project types.*

## Overview

The Detailed Goals is the foundation of spec-driven development, where rough feature ideas are transformed into clear, testable requirements using the EARS (Easy Approach to Requirements Syntax) format. This phase ensures all stakeholders have a shared understanding of what needs to be built before moving to design and implementation.

## Purpose and Goals

The detailed goals serves to:
...
> Complete this section when the feature creates, reads, updates, deletes, imports, exports, or derives persisted data. Keep this at the requirement level; table/schema design belongs in `detailed-design.md`.

- **Persisted data involved**: [business entities / records that must be stored or read]
- **Data ownership**: [workspace / project / user / global / external system]
- **Lifecycle**: [create/update/delete/archive/restore/retention expectations]
- **Consistency requirements**: [uniqueness, ordering, idempotency, conflict handling, transactional expectations]
- **Query and reporting needs**: [filters, sorting, pagination, aggregation, search]
- **Volume and growth assumptions**: [expected rows, request frequency, historical retention]
- **Migration/backfill expectations**: [new data only / existing data migration / backfill / no migration]
- **Audit and compliance needs**: [audit trail, PII handling, soft delete, access logging]
...
- **Notes / required revisions before design**: [...]
```
...
- [ ] Data and persistence scope is completed when the feature touches stored data
- [ ] Data ownership, lifecycle, consistency, query, and migration expectations are captured when applicable
- [ ] Approval Gate is present and remains `Pending Confirmation` until the user/stakeholder explicitly approves moving to design
